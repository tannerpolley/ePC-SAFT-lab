...
from .hessian_agreement import exact_hessian_target, hessian_baseline_status
        ...
        rows.append(
            {
                ...
                "exact_baseline": baseline_status,
...
def _baseline_status(target: str) -> str:
    if target in {"density", "strength", "composition"}:
        return "exact_implicit_first_derivative"
    return hessian_baseline_status()
...
def main() -> None:
    ...
    if args.emit_json:
        print(json.dumps(run_jax_picard_derivative_cases()))
        return
