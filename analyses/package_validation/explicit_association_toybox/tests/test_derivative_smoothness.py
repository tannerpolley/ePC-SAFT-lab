...
def test_derivative_smoothness_rows_cover_local_coordinates() -> None:
    rows = derivative_smoothness_rows(
        closure_names=("implicit_exact_mass_action", "explicit_damped_picard_unroll_3"),
        density=0.05,
        strength=10.0,
    ...
    assert {"implicit_exact_mass_action", "explicit_damped_picard_unroll_3"} <= {row["closure_name"] for row in rows}
