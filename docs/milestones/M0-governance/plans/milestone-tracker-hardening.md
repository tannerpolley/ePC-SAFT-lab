...
`docs/milestones/` for durable plans, optional issue handoffs, and milestone
owned registries while GitHub Issues and the `ePC-SAFT Roadmap` Project remain
authoritative for live state.

## Acceptance Gates

- [x] GitHub milestone descriptions and `docs/milestones/PROJECT_CONTEXT.md`
      milestone descriptions match for the active milestone taxonomy.
- [x] Closed GitHub issue mirrors are removed from `docs/milestones/M*/issues/`.
- [x] Local issue mirrors that remain all point to open GitHub issues.
- [x] `docs/milestones/_templates/` contains reusable plan and issue-mirror
      templates.
