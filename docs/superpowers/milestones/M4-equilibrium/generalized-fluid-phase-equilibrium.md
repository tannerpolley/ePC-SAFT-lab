...
where code came from; it does not prove the local `.so` imported by Python was
...
The script also supports `--generate-phase-discovery` for an intentional
one-command diagnostic run; tests should prefer a supplied phase-discovery
payload so neutral TP flash coverage does not hide a second TPD/Ipopt route
solve.
