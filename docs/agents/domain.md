...
When suggesting or documenting package entrypoints, prefer the current public seams recorded in `CONTEXT.md` and the ADRs: `Equilibrium(mixture, route=..., ...).solve()` for production neutral VLE equilibrium, canonical parameter families through `ParameterSet`, shared regression summaries through `TargetDataset.target_family_summaries()`, and capability claims through `epcsaft.capabilities()`.
