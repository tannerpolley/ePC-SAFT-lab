...
.. list-table::
   ...
   * - Stage 9 phase-discovery evidence
     ...
     - Cheap Stage 9 phase-discovery snapshot for deterministic screening, continuous TPD, HELD Stage I, and Stage II candidate bound-gap diagnostics. Add ``--include-route-refinement`` only when current-route Stage III Ipopt evidence is needed; add ``--debug --include-route-refinement --require-complete`` to print continuous-TPD trace rows, route seed-attempt markers, Ipopt ``print_level=5`` output, and bounded Ipopt iteration-history diagnostics for this one evidence route while failing on incomplete convergence evidence.
   * - Docs check
...
- Equilibrium convergence diagnosis: use one explicit equilibrium test node with ``uv run python run_pytest.py --equilibrium-debug -q -s <target>`` or the Stage 9 evidence script with ``--debug --include-route-refinement --require-complete``. Do not use generic/API slices to investigate equilibrium runtime; they intentionally do not run the full ``tests/api/frontend/test_equilibrium.py`` route-solve file or native result-file sweeps under ``tests/native/equilibrium/results``. The wrapper rejects broad targets, slice-backed debug runs, and multi-node debug runs; long-suite opt-in is for deliberate sweep validation, not debug diagnosis. The Stage 9 script records bounded Ipopt iteration history in its diagnostics payload so convergence can be audited without broad pytest runs. For Stage 10 neutral proof diagnosis, run ``uv run python scripts/validation/check_stage10_neutral_tp_flash_proof.py --stage9-evidence-json <payload> --json --debug --require-proof``; debug native output is routed to stderr and JSON proof evidence stays on stdout.
