...
.. list-table::
   ...
   * - Equilibrium debug trace
     - ``uv run python run_pytest.py --equilibrium-debug -q -s <one equilibrium test node>`` or ``uv run python run_pytest.py --equilibrium-confidence --equilibrium-debug -q -s``
     - Opt-in verbose Ipopt print level, continuous-TPD trace output, and expanded iteration-history capture for diagnosing iteration-limit, TPD candidate, or seed-attempt behavior. Debug mode requires either one explicit equilibrium test node or an explicit ``--equilibrium-confidence`` request; it does not silently expand into a multi-test run.
   * - Stage 9 phase-discovery evidence
     - ``uv run python scripts/validation/check_stage9_phase_discovery_evidence.py --json``
     - Cheap Stage 9 phase-discovery snapshot for deterministic screening, continuous TPD, HELD Stage I, and Stage II candidate bound-gap diagnostics. Add ``--include-route-refinement`` only when current-route Stage III Ipopt evidence is needed; add ``--debug --include-route-refinement`` to print continuous-TPD trace rows and Ipopt ``print_level=5`` output for this one evidence route.
   * - Docs check
...
example, read ``docs/roadmaps/generalized_fluid_phase_equilibrium.md`` before
...
- Equilibrium convergence diagnosis: use one explicit equilibrium test node with ``uv run python run_pytest.py --equilibrium-debug -q -s <target>``. Use ``--equilibrium-confidence --equilibrium-debug`` only when intentionally running the focused confidence slice. Do not use generic/API slices to investigate equilibrium runtime; they intentionally do not run the full ``tests/api/frontend/test_equilibrium.py`` route-solve file or native result-file sweeps under ``tests/native/equilibrium/results``. The wrapper rejects broad targets and multi-node debug runs; long-suite opt-in is for deliberate sweep validation, not debug diagnosis.
