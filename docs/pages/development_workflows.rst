...
- Equilibrium convergence diagnosis: use one explicit equilibrium test node with ``uv run python run_pytest.py --equilibrium-debug -q -s <target>``. Do not use generic/API slices to investigate equilibrium runtime; they intentionally do not run the full ``tests/api/frontend/test_equilibrium.py`` route-solve file or native result-file sweeps under ``tests/native/equilibrium/results``. The wrapper rejects broad targets, slice-backed debug runs, and multi-node debug runs; long-suite opt-in is for deliberate sweep validation, not debug diagnosis.
