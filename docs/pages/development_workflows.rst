...
.. list-table::
   ...
   * - Stage 9 phase-discovery evidence
     ...
     - Cheap Stage 9 phase-discovery snapshot for deterministic screening, continuous TPD, HELD Stage I, and Stage II candidate bound-gap diagnostics. Add ``--include-route-refinement`` only when current-route Stage III Ipopt evidence is needed; add ``--debug --include-route-refinement --require-complete`` to print continuous-TPD trace rows, route seed-attempt markers, and Ipopt ``print_level=5`` output for this one evidence route while failing on incomplete convergence evidence.
   * - Docs check
