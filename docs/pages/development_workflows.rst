...
.. list-table::
   ...
   * - Equilibrium debug trace
     ...
     - Focused neutral TP flash proof with opt-in verbose Ipopt print level, continuous-TPD trace output, and expanded iteration-history capture for diagnosing iteration-limit, TPD candidate, or seed-attempt behavior. With no explicit target, ``--equilibrium-debug`` defaults to the focused confidence slice.
   * - Docs check
