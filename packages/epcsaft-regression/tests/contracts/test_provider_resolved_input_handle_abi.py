...
import epcsaft
import epcsaft_regression._native_core as regression_core
import pytest
...
def _provider_handle():
    domain = epcsaft.TemperatureDomain(
        100.0,
        500.0,
        epcsaft.DomainEvidence("source_validity", "Stage 4 regression ABI fixture"),
    )
    source = epcsaft.ScientificSource("literature", "Stage 4 regression ABI fixture")
    values = {
...
def test_regression_retains_and_reads_the_provider_owned_handle() -> None:
    handle = _provider_handle()
    detached = {
        "schema": handle.schema,
        "snapshot_fingerprint_sha256": handle.snapshot_fingerprint_sha256,
    }
    expected = regression_core._native_provider_resolved_input_handle_probe(handle)
    del detached
    gc.collect()

    assert regression_core._native_provider_resolved_input_handle_probe(handle) == expected
    assert expected["contract_id"] == "provider_resolved_input_handle_v1"
    assert expected["component_order"] == ["Methane"]
    with pytest.raises(TypeError):
        regression_core._native_provider_resolved_input_handle_probe(expected)
