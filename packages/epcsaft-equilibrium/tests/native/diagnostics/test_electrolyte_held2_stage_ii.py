...
def test_electrolyte_held2_stage_ii_consumes_stage_i_and_reports_open_bound_gap() -> None:
    ...
    assert payload["blockers"] == [
        ...
        "stage_ii_bound_gap_open",
        "stage_ii_dual_loop_incomplete",
        "stage_ii_dual_loop_unverified",
        "stage_ii_replay_payload_missing",
    ...
    assert evidence["stage_ii_status"] == "dual_loop_incomplete"
    assert evidence["candidate_bound_audit_status"] == "candidate_bound_gap_open"
    assert evidence["dual_loop_status"] == "incomplete_candidate_bound_gap_open"
    assert evidence["stopping_reason"] == "bound_gap_open"
    ...
    assert bounds["bound_gap"] > bounds["bound_tolerance"]
    assert bounds["bound_gap"] == 0.3497537959766108
...
def test_electrolyte_held2_stage_ii_retains_reduced_basis_coverage() -> None:
    ...
    assert payload["complete"] is False
    assert "stage_ii_bound_gap_open" in payload["blockers"]
    ...
    for _key, evidence in coverage.items():
