...
- Hard rule: use IntelliJ MCP first by default for repo work. Start with `intellij-index` for file lookup, text lookup, symbols, definitions, references, safe refactors, and diagnostics when the IDE is ready. Try the relevant indexed action at least twice when the first result is stale, empty, ambiguous, or an indexing-ready retry is plausible before falling back to `rg`, `Get-Content`, `Get-ChildItem`, or other manual shell inspection.
