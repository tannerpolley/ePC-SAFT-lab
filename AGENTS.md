...
- After approval, keep automation minimal: launch or focus IntelliJ with this repo opened as its own project, pass `C:\Users\Tanner\Documents\Workspaces\Engineering\ePC-SAFT` as MCP `projectPath`, wait for indexing readiness, then use MCP tools.
- Do not route repo-specific commands, run configurations, semantic navigation, or diagnostics through the Engineering workspace root.
- Treat `intellij-index` as an IDE-backed index over open IntelliJ projects, not as a generic filesystem indexer.
- Hard rule: use IntelliJ MCP first by default for repo work. Start with the full enabled `intellij-index` toolset when the IDE is ready: status/sync/open/active-file, indexed read/search/file/class/symbol lookup, definitions, references, implementations, super methods, call hierarchy, type hierarchy, diagnostics, formatting/import cleanup, semantic rename/move/safe-delete, and IDE build only when useful for the current project model. If only a subset of Index MCP tools is visible, call `tool_search` for the missing relevant tool before falling back.
- Try the relevant indexed action at least twice when the first result is stale, empty, ambiguous, or an indexing-ready retry is plausible before falling back to `rg`, `Get-Content`, `Get-ChildItem`, or other manual shell inspection.
- Hard rule: use `jetbrains-bundled` run-configuration MCP first by default for durable scripts, tests, validation commands, build commands, docs/report commands, analysis commands, package commands, and maintenance commands. List shared run configurations at the open project root, run the named configuration when it exists, and do not run an equivalent ad hoc `uv run python ...` or PowerShell command unless the MCP run/config path is unavailable after a couple of concrete attempts or no durable config exists yet.
- If a durable command is missing from Services, update `scripts/dev/configure_jetbrains_project.py` and `.run/*.run.xml` first, run the dry-run/apply normalizer configs, then execute the new named run configuration through MCP. Use shell only for one-off probes, raw config reads after MCP lookup, git operations, or bootstrapping/fixing the run dashboard when IntelliJ cannot see it yet.
...
- Services grouping is owned by the open IntelliJ project. In the standalone `ePC-SAFT` project, shared `.run` configs must not set `folderName`, because that creates a redundant nested `ePC-SAFT` folder in Services. Only use a single-level repo folder such as `ePC-SAFT` when intentionally configuring a multi-repo Engineering workspace view.
...
- For non-trivial code work, public API or native wrapper changes, architecture questions, bug diagnosis, deletion/rename/move work, or review of shared behavior, use every relevant `intellij-index` action family before finalizing when the IDE is ready, not just one token lookup.
- Prefer IntelliJ-backed semantic navigation and diagnostics when symbol meaning matters:
  - indexed project reads and lookup: `ide_read_file`, `ide_search_text`, `ide_find_file`, `ide_find_class`, `ide_find_symbol`, `ide_file_structure`
  - semantic navigation: `ide_find_definition`, `ide_find_references`, `ide_find_implementations`, `ide_find_super_methods`, `ide_call_hierarchy`, `ide_type_hierarchy`
  - file analysis, build errors, and test result diagnostics
  - diagnostics and editor state: `ide_diagnostics`, `ide_get_active_file`, `ide_open_file`, `ide_sync_files`
  - safe edits/refactors: `ide_optimize_imports`, `ide_reformat_code`, `ide_refactor_rename`, `ide_move_file`, `ide_refactor_safe_delete`
  - `ide_build_project` only when the IntelliJ project has a meaningful IDE build target; for `ePC-SAFT`, prefer shared Services run configs for native build, pytest, docs, package, and validation proof because the Index MCP build action can abort without proving the package build.
